...
function ClientPanel({data,goStore}:{data:Data;goStore:()=>void}){
  ...
  const titles:Record<string,string>={overview:`OLÁ, ${(data.session.user?.displayName||"").split(" ")[0].toUpperCase()}`,assessment:"MINHA AVALIAÇÃO",nutrition:"PLANO ALIMENTAR",training:"TREINO PERSONALIZADO",orders:"MINHAS COMPRAS",tutorial:"TUTORIAL"};
  return <section className="portal"><aside className="portal-nav"><div className="profile"><i>{initials(data.session.user?.displayName||"B7")}</i><div><small>CLIENTE B7</small><b>{data.session.user?.displayName}</b></div></div>{[["overview","VISÃO GERAL"],["assessment","MINHA AVALIAÇÃO"],["nutrition","ALIMENTAÇÃO"],["training","MEU TREINO"],["orders","MINHAS COMPRAS"],["tutorial","?  TUTORIAL"]].map(x=><button key={x[0]} className={tab===x[0]?"active":""} onClick={()=>setTab(x[0])}>{x[1]}<span>→</span></button>)}<button onClick={goStore}>IR PARA A LOJA ↗</button></aside><div className="portal-content">
    <div className="portal-head"><div><p className="kicker">ÁREA DO CLIENTE</p><h1>{titles[tab]}</h1></div><span>DADOS PRIVADOS</span></div>
    ...
    {tab==="tutorial"&&<Tutorial role="client" onOpen={target=>target==="store"?goStore():setTab(target)}/>}
  </div></section>
}
...
function Tutorial({role,onOpen}:{role:"client"|"admin";onOpen:(target:string)=>void}){
  const clientSteps=[
    {n:"01",title:"Comece pela visão geral",text:"Veja rapidamente seu IMC, gordura corporal e quais planos já estão ativos.",target:"overview",action:"ABRIR VISÃO GERAL"},
    {n:"02",title:"Confira sua avaliação",text:"Acompanhe medidas, composição corporal e o parecer enviado pelo profissional.",target:"assessment",action:"VER AVALIAÇÃO"},
    {n:"03",title:"Siga sua alimentação",text:"Consulte horários, refeições, porções e calorias definidas para você.",target:"nutrition",action:"VER ALIMENTAÇÃO"},
    {n:"04",title:"Execute seu treino",text:"Veja exercícios, séries, repetições e cargas sem precisar procurar mensagens antigas.",target:"training",action:"VER TREINO"},
    {n:"05",title:"Acompanhe as compras",text:"Consulte pedidos, pagamentos e o andamento de cada compra realizada.",target:"orders",action:"VER COMPRAS"},
    {n:"06",title:"Precisa de suplementos?",text:"Volte à loja para escolher os produtos indicados para sua rotina.",target:"store",action:"IR PARA A LOJA"}
  ];
  const adminSteps=[
    {n:"01",title:"Cadastre os clientes",text:"Comece criando o perfil do aluno com nome, contato, objetivo e situação atual.",target:"clients",action:"ABRIR CLIENTES"},
    {n:"02",title:"Faça a avaliação",text:"Registre peso, altura e circunferências. O sistema calcula todos os indicadores.",target:"assessments",action:"ABRIR AVALIAÇÕES"},
    {n:"03",title:"Monte alimentação e treino",text:"Use a busca para selecionar alimentos e exercícios e envie o plano ao cliente.",target:"plans",action:"MONTAR PLANO"},
    {n:"04",title:"Organize os produtos",text:"Cadastre fotos, preços, estoque, categorias e destaque dos itens da loja.",target:"products",action:"ABRIR PRODUTOS"},
    {n:"05",title:"Acompanhe os pedidos",text:"Atualize o status desde o pagamento até a entrega para manter o cliente informado.",target:"orders",action:"ABRIR PEDIDOS"},
    {n:"06",title:"Use a visão geral",text:"Monitore clientes, pedidos, faturamento e produtos ativos em um só lugar.",target:"overview",action:"VER RESUMO"}
  ];
  const steps=role==="admin"?adminSteps:clientSteps;
  return <section className="tutorial"><div className="tutorial-intro"><span>GUIA RÁPIDO</span><h2>{role==="admin"?"COMO USAR O PAINEL ADM":"COMO USAR SEU PAINEL"}</h2><p>{role==="admin"?"Siga esta ordem para cadastrar, avaliar e acompanhar cada cliente com segurança.":"Tudo o que você precisa está organizado abaixo. Escolha uma etapa para ir direto ao conteúdo."}</p></div><div className="tutorial-grid">{steps.map(step=><article key={step.n}><small>{step.n}</small><h3>{step.title}</h3><p>{step.text}</p><button onClick={()=>onOpen(step.target)}>{step.action}<span>→</span></button></article>)}</div><aside className="tutorial-tip"><b>DICA B7</b><p>{role==="admin"?"O fluxo recomendado é: cliente → avaliação → alimentação e treino → acompanhamento.":"Seu painel é atualizado pelo profissional. Se algum conteúdo ainda não aparecer, aguarde o próximo envio ou entre em contato com a B7."}</p></aside></section>
}
...
function AdminPanel({data,post,busy,notify}:{data:Data;post:(p:unknown)=>Promise<unknown>;busy:boolean;notify:(s:string)=>void}){
  ...
  return <section className="admin-shell"><aside className="admin-nav"><div><Mark compact/><small>CONTROLE CENTRAL</small></div>{[["overview","VISÃO GERAL"],["products","PRODUTOS"],["clients","CLIENTES"],["orders","PEDIDOS"],["assessments","AVALIAÇÕES"],["plans","ALIMENTAÇÃO E TREINO"],["tutorial","?  TUTORIAL"]].map(x=><button key={x[0]} className={tab===x[0]?"active":""} onClick={()=>setTab(x[0])}>{x[1]}<span>→</span></button>)}</aside><div className="admin-content"><div className="admin-head"><div><p className="kicker">PAINEL ADMINISTRATIVO</p><h1>{tab.toUpperCase()}</h1></div><span>ACESSO DO PROPRIETÁRIO</span></div>
    ...
    {tab==="tutorial"&&<Tutorial role="admin" onOpen={setTab}/>}
  </div></section>
}
